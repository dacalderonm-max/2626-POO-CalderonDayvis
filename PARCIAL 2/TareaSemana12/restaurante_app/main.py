...
def guardar_datos(restaurante: Restaurante, servicio: ArchivoServicio) -> None:
    ...
    if ok_productos and ok_usuarios and ok_ventas:
        print("\nOK: Todos los datos se han guardado exitosamente.")
    else:
...
def cargar_datos(restaurante: Restaurante, servicio: ArchivoServicio) -> None:
    ...
    if ok_ventas:
        restaurante.establecer_ventas(ventas)

    if ok_productos and ok_usuarios and ok_ventas:
        print("\nOK: Datos cargados correctamente desde JSON.")
    else:
        print("\nERROR: Ocurrió un error al cargar algunos datos.")


def limpiar_datos(restaurante: Restaurante) -> None:
    restaurante.limpiar_datos()
    print("\nOK: Datos en memoria eliminados.")
...
def ejecutar_programa() -> None:
    ...
    while True:
        ...
        if opcion == "1":
            ...
            guardar_datos(restaurante, archivo_servicio)
            print("\nOK: Datos guardados automáticamente. Gracias por usar el sistema de restaurante!")
            break
        else:

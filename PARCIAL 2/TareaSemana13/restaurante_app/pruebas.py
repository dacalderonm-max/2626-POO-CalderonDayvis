...
def limpiar_archivos(gestor: GestorDatos) -> None:
    ...
    print("[OK] Archivos anteriores eliminados")
...
def prueba_modelos() -> None:
    ...
    print(f"\n[OK] Producto creado: {producto.mostrar_informacion()}")

    # Convertir a diccionario
    producto_dict = producto.a_diccionario()
    print(f"[OK] Convertido a diccionario: {producto_dict}")

    # Recuperar desde diccionario
    producto_recuperado = Producto.desde_diccionario(producto_dict)
    print(f"[OK] Recuperado desde diccionario: {producto_recuperado.mostrar_informacion()}")

    # Crear un cliente
    cliente = Cliente("1234567890", "Juan Pérez", "juan@example.com")
    print(f"\n[OK] Cliente creado: {cliente.mostrar_informacion()}")

    # Convertir a diccionario
    cliente_dict = cliente.a_diccionario()
    print(f"[OK] Convertido a diccionario: {cliente_dict}")

    # Recuperar desde diccionario
    cliente_recuperado = Cliente.desde_diccionario(cliente_dict)
    print(f"[OK] Recuperado desde diccionario: {cliente_recuperado.mostrar_informacion()}")
...
def prueba_persistencia() -> None:
    ...
    print(f"[OK] {restaurante.obtener_cantidad_productos()} productos registrados en memoria")
    ...
    print(f"[OK] {restaurante.obtener_cantidad_clientes()} clientes registrados en memoria")
    ...
    if os.path.exists(gestor.ruta_productos):
        print(f"[OK] Archivo de productos existe: {gestor.ruta_productos}")
        ...
        print(f"[ERROR] Archivo de productos no encontrado")

    if os.path.exists(gestor.ruta_clientes):
        print(f"[OK] Archivo de clientes existe: {gestor.ruta_clientes}")
        ...
        print(f"[ERROR] Archivo de clientes no encontrado")
...
def prueba_carga() -> None:
    ...
    print("\n--- Datos Cargados ---")
    print(f"[OK] Productos cargados: {restaurante.obtener_cantidad_productos()}")
    for p in restaurante.listar_productos():
        print(f"  - {p}")

    print(f"\n[OK] Clientes cargados: {restaurante.obtener_cantidad_clientes()}")
...
def prueba_estructura_json() -> None:
    ...
    try:
        with open(gestor.ruta_productos, 'r', encoding='utf-8') as f:
            ...
            if productos:
                print(f"[OK] Primer producto: {json.dumps(productos[0], indent=2, ensure_ascii=False)}")
    ...
    try:
        with open(gestor.ruta_clientes, 'r', encoding='utf-8') as f:
            ...
            print("[OK] JSON válido")
            if clientes:
                print(f"[OK] Primer cliente: {json.dumps(clientes[0], indent=2, ensure_ascii=False)}")
    except Exception as e:
        print(f"[ERROR] Error: {e}")
